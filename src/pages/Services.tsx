...
export default function Services() {
  ...
  const categories = ['Tất Cả', 'Váy Cưới', 'Chụp Ảnh', 'Địa Điểm', 'Trang Trí', 'Trang Điểm'];
  ...
  useEffect(() => {
    async function fetchVendors() {
      try {
        const { data, error } = await supabase.from('vendors').select('*, categories(name)');
        ...
        if (data && data.length > 0) {
          const mappedVendors: Vendor[] = data.map((v: any) => ({
            id: v.id,
            name: v.name,
            category: v.categories?.name || 'Khác',
            rating: v.rating,
            addr: v.address,
            ...
            <div className="flex overflow-x-auto no-scrollbar w-full md:w-auto pb-2 md:pb-0 gap-2">
              {categories.map(cat => (
                <button
                  ...
                  className={`px-6 py-2.5 rounded-full text-xs font-bold tracking-widest whitespace-nowrap transition-all uppercase border ${
                    activeCategory === cat 
