    ...
    async function fetchPosts() {
      try {
        ...
        const formattedPosts: BlogPost[] = await Promise.all((postsData as any[]).map(async (post: any) => {
          ...
          if (post.user_id) {
            // v2: profiles instead of users
            const { data: profileData } = await supabase.from('profiles').select('username, avatar_url').eq('id', post.user_id).single();
            if (profileData) {
        ...
        <main className="flex-1 flex flex-col gap-6 max-w-[650px] mx-auto w-full pb-10">
          <div className="bg-[#FFFFFF] rounded-[32px] pt-4 px-6 pb-0 shadow-sm border border-rose-50 flex flex-col">
            <div className="flex items-center gap-3 mb-4 bg-white border border-rose-100 rounded-full p-2 pl-4 pr-3 shadow-sm">
              ...
              <input type="text" placeholder="Bạn đang nghĩ gì?" className="flex-1 bg-transparent border-none outline-none text-sm placeholder:text-[#1B2C40]/40 text-[#1B2C40] font-medium" />
            ...
            {loading ? (
              ...
              <div key={post.id} className="bg-[#FFFFFF] rounded-[32px] p-6 shadow-sm border border-rose-50 flex flex-col gap-4">
                <div className="flex items-center gap-3">
                  ...
                  <button className="ml-auto text-[#1B2C40]/40 hover:text-[#1B2C40]"><MoreHorizontal className="w-5 h-5" /></button>
                </div>
