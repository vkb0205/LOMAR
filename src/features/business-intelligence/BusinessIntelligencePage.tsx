...
export default function BusinessIntelligence() {
  ...
  return <div className="min-h-[70vh] bg-[#f7f8fa] px-4 pb-8 pt-24 text-slate-900 sm:px-8 sm:pt-28 lg:px-12"><div className="mx-auto max-w-[1440px]">
    ...
    <section className="mt-7 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{data.metrics.map((metric, index) => { const Icon = icons[index % icons.length]; return <div key={metric.label} className="rounded-2xl border border-slate-200 bg-white p-5"><div className="flex justify-between"><p className="text-xs font-semibold uppercase tracking-[0.12em] text-slate-500">{metric.label}</p><Icon className="h-4 w-4 text-orange-600" /></div><p className="mt-4 text-2xl font-bold">{metric.value}</p><p className={`mt-2 text-xs font-semibold ${metric.positive ? 'text-rose-600' : 'text-rose-600'}`}>{metric.change} vs. previous period</p></div>; })}</section>
    <div className="mt-5 grid gap-5 xl:grid-cols-[1.5fr_1fr]"><section className="rounded-2xl border border-slate-200 bg-white p-5"><h2 className="font-bold">Revenue overview</h2><p className="mt-1 text-xs text-slate-500">Gross merchandise value over time</p><div className="mt-4"><RevenueChart data={data.trend} /></div></section><section className="rounded-2xl border border-slate-200 bg-white p-5"><h2 className="font-bold">Top categories</h2><p className="mt-1 text-xs text-slate-500">Revenue contribution</p><div className="mt-4"><CategoryPieChart data={data.categories} /></div></section></div>
    <div className="mt-5 grid gap-5 lg:grid-cols-2 xl:grid-cols-3">
      <section className="rounded-2xl border border-slate-200 bg-white p-5"><h2 className="font-bold">Lead sources</h2><p className="mt-1 text-xs text-slate-500">Inquiries by channel</p><div className="mt-4"><LeadSourceChart /></div></section>
      <section className="rounded-2xl border border-slate-200 bg-white p-5"><h2 className="font-bold">Conversion funnel</h2><p className="mt-1 text-xs text-slate-500">Visitor to completed booking</p><div className="mt-4"><ConversionFunnel /></div></section>
      <section className="rounded-2xl border border-slate-200 bg-white p-5 lg:col-span-2 xl:col-span-1"><h2 className="font-bold">Inquiry heatmap</h2><p className="mt-1 text-xs text-slate-500">Weekly activity by day and hour</p><div className="mt-4"><WeeklyHeatmap /></div></section>
    </div>
