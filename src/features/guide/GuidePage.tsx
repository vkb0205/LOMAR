  ...
  return (
    <div className="w-full flex flex-col font-sans pb-20 bg-[#FFFFFF] min-h-screen">
      ...
      <div className="relative w-full h-[400px] flex items-center justify-center overflow-hidden bg-[#1B2C40]">
        ...
        />
        <div className="absolute inset-0 bg-gradient-to-t from-[#1B2C40] to-transparent" />
      ...
      <div className="max-w-[1200px] w-full mx-auto px-4 mt-[-40px] relative z-20 flex flex-col lg:flex-row gap-8">
        ...
        <div className="flex-1 bg-white rounded-[32px] shadow-sm border border-rose-100 p-8 md:p-10">
          <div className="flex items-center justify-between mb-8 pb-6 border-b border-rose-100">
            <div>
              <h2 className="text-2xl font-serif font-bold text-[#1B2C40] uppercase tracking-widest mb-2">Checklist Chuẩn Bị</h2>
              <p className="text-[#6B92B4] text-xs font-medium">Lộ trình chi tiết cho đám cưới trong mơ</p>
            </div>
          ...
          <div className="flex flex-col md:flex-row gap-8">
            ...
            <div className="w-full md:w-48 flex flex-col gap-3 shrink-0">
              {timelineData.map((data, idx) => (
                <button
                  ...
                  className={`text-left px-5 py-4 rounded-2xl transition-all border ${activePhase === idx
                      ? 'bg-[#1B2C40] text-white border-[#1B2C40] shadow-md'
                      : 'bg-white text-[#6B92B4] border-rose-100 hover:border-[#1B2C40] hover:text-[#1B2C40]'
                    }`}
                >
            ...
            <motion.div
              ...
              className="flex-1 bg-[#FFFFFF] rounded-2xl border border-rose-50 p-6 md:p-8"
            >
              <div className="flex items-center gap-3 mb-6">
                <Calendar className="w-6 h-6 text-[#F2BFC8]" />
                <h3 className="text-xl font-serif font-bold text-[#1B2C40]">{timelineData[activePhase].phase}</h3>
              </div>
              ...
              <div className="space-y-4">
                {timelineData[activePhase].tasks.map((task, idx) => (
                  <div key={idx} className="flex items-start gap-4 p-4 bg-white rounded-xl shadow-sm border border-rose-50 group cursor-pointer hover:border-rose-200 transition-colors">
                    <div className="mt-0.5 text-rose-300 group-hover:text-[#F2BFC8] transition-colors">
                      <CheckCircle2 className="w-5 h-5" />
                    </div>
                    <p className="text-[#1B2C40] text-sm font-medium leading-relaxed pt-0.5">{task}</p>
                  </div>
                ))}
              </div>
            </motion.div>
        ...
        <aside className="lg:w-[380px] flex flex-col gap-6 shrink-0">
          ...
          <div className="bg-white rounded-[32px] shadow-sm border border-rose-100 overflow-hidden flex flex-col group cursor-pointer">
            <div className="h-48 w-full relative overflow-hidden">
              ...
              />
              <div className="absolute top-4 left-4 bg-white/90 backdrop-blur-sm px-3 py-1 rounded-full text-[10px] font-bold text-[#1B2C40] uppercase tracking-widest">
                Mẹo Hay
              </div>
            </div>
            <div className="p-6">
              <h3 className="font-serif font-bold text-lg text-[#1B2C40] mb-3 leading-tight group-hover:text-[#F2BFC8] transition-colors">
                5 Lưu ý quan trọng khi chọn địa điểm tổ chức tiệc cưới
              </h3>
              <p className="text-[#6B92B4] text-xs leading-relaxed mb-4 line-clamp-2">
                Đừng vội đặt cọc ngay nếu bạn chưa kiểm tra kỹ các yếu tố về không gian, âm thanh, ánh sáng và bãi đỗ xe...
              </p>
              <button className="flex items-center text-[#F2BFC8] text-[10px] font-bold uppercase tracking-widest hover:text-rose-500 transition-colors">
          ...
          {/* Video Tutorial */}
          <div className="bg-[#1B2C40] rounded-[32px] shadow-sm overflow-hidden flex flex-col relative p-8">
            ...
            <div className="relative z-10 flex flex-col items-center text-center h-full justify-center">
              ...
              <button className="w-full py-3 bg-[#F2BFC8] text-white rounded-full font-bold text-[10px] uppercase tracking-widest shadow-lg shadow-rose-500/30 hover:-translate-y-0.5 hover:bg-rose-400 transition-all">
                XEM VIDEO NGAY
              </button>
            </div>
          </div>
