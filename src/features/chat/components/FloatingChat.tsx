...
export default function FloatingChat() {
  ...
  useEffect(() => {
    // Session continuity is server-side via /chat/consult sessionId. Local state
    // resets on auth change without inventing broken thread ids.
      ...
      {isOpen && (
        <AnimatePresence>
          <motion.div
            ...
            className="fixed bottom-6 right-6 z-50 flex h-[520px] max-h-[85dvh] w-[380px] max-w-[95vw] flex-col overflow-hidden rounded-bezel bg-ink/5 p-1.5 ring-1 ring-ink/5 shadow-float"
          >
            ...
            <div className="flex-1 space-y-4 overflow-y-auto bg-canvas/70 p-4">
              {messages.map((msg, i) => (
