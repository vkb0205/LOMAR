...
| 10 | Migration not safely reproducible from source | ✅ 🛠️ | [`database/migrate_to_v2.sql:17`](database/migrate_to_v2.sql:17) `create extension if not exists "uuid-ossp";` and [`migrate_to_v2.sql:18`](database/migrate_to_v2.sql:18) `create extension if not exists pgcrypto;` now run **before** SECTION 1; the old SECTION 2 statement is replaced with a note ([`migrate_to_v2.sql:327`](database/migrate_to_v2.sql:327)) |
| 11 | Migration script keeps active destructive drops | ✅ 🛠️ | All 21 `drop table if exists ... cascade;` statements in SECTION 8 ([`migrate_to_v2.sql:1196`](database/migrate_to_v2.sql:1196)–[`migrate_to_v2.sql:1216`](database/migrate_to_v2.sql:1216)) are now commented out with `-- `; closing instruction added at [`migrate_to_v2.sql:1217`](database/migrate_to_v2.sql:1217) |
| 12 | Post-migration RLS coverage is incomplete | ✅ | [`database/migrate_to_v2.sql:741`](database/migrate_to_v2.sql:741) "SECTION 6b" adds full owner-scoped CRUD for all user-owned tables |
| 13 | Code relies heavily on `as any` around DB access | ✅ 🛠️ | Root cause fixed in [`src/types/database.ts`](src/types/database.ts:1) (`Relationships: []` on all 20 tables + Views/Functions keys at [`database.ts:739`](src/types/database.ts:739)); all `as any` DB write casts removed across Customize, FloatingChat, AIConsultant, Dashboard, AppContext — see detail below |
...
[`database/migrate_to_v2.sql:741`](database/migrate_to_v2.sql:741) "SECTION 6b" defines an explicit ownership model (`profiles.id == auth.uid()`; user-owned tables via `user_id`; vendor-scoped via `vendors.owner_id` `EXISTS` sub-selects) and grants full owner-scoped CRUD on every user-owned table. All statements replay-safe (`drop policy if exists ... ; create policy ...`).

## Fixed this pass — detail by subtask

### Subtask 1 — Migration cleanup (items 10, 11)

**Item 10 — extension ordering.** [`database/migrate_to_v2.sql:17`](database/migrate_to_v2.sql:17)–[`migrate_to_v2.sql:18`](database/migrate_to_v2.sql:18) now run `create extension if not exists "uuid-ossp";` and `create extension if not exists pgcrypto;` at the very top of the file, **before** SECTION 1 table defaults and SECTION 3 data migration use `gen_random_uuid()` / `gen_random_bytes()`. The old SECTION 2 enable statement was removed and replaced with an explanatory note at [`migrate_to_v2.sql:327`](database/migrate_to_v2.sql:327). A clean/staging DB replay no longer fails on missing extension functions.

**Item 11 — destructive drops neutralized.** All 21 `drop table if exists ... cascade;` statements in SECTION 8 ([`migrate_to_v2.sql:1196`](database/migrate_to_v2.sql:1196)–[`migrate_to_v2.sql:1216`](database/migrate_to_v2.sql:1216)) are now prefixed with `-- ` so they are inert. A closing instruction at [`migrate_to_v2.sql:1217`](database/migrate_to_v2.sql:1217) tells operators to uncomment and run manually only after verifying data migration. Re-running the script no longer risks data loss.
...
| File | Items | Change |
|---|---|---|
| [`database/migrate_to_v2.sql`](database/migrate_to_v2.sql:1) | 10, 11 | Extensions moved to top; SECTION 8 drops commented out |
...
| [`src/types/database.ts`](src/types/database.ts:1) | 13 | `Relationships: []` on all tables + Views/Functions keys |
